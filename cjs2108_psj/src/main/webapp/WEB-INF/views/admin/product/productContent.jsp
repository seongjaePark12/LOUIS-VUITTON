...
<title>${productVo.productName}</title>
	...
	<script src="${ctp}/js/productContent.js"></script>
</head>
<body oncontextmenu='return false' ondragstart='return false'>
	...
	<div class="w3-main" style="margin-left:340px;margin-right:40px">
		<div class="w3-container" style="margin-top:80px" id="showcase">
			<form name="myform" method="post" enctype="multipart/form-data">
				<div class="form-group" style="float:left;width: 31%; margin-right: 35px;">
					...
					<select name="categoryMainCode" id="categoryMainCode" class="form-control" onchange="categoryMainChange()">
						<option value="${productVo.categoryMainCode}">${productVo.categoryMainCode}</option>
			   ...
			   <div class="form-group" style="float:left;width: 31%; margin-right: 35px;">
			      ...
			      <select id="categoryMiddleCode" name="categoryMiddleCode" class="form-control" onchange="categoryMiddleChange()">
			       <option value="${productVo.categoryMiddleCode}">${productVo.categoryMiddleCode}</option>
			   ...
			   <div class="form-group" style="float:left;width: 31%;">
			      ...
			      <select id="categorySubCode" name="categorySubCode" class="form-control">
			    	  <option value="${productVo.categorySubCode}">${productVo.categorySubCode}</option>
		       ...
		       <input type="text" name="productCode" class="form-control" id="productCode" value="${productVo.productCode}" required >
		     </div>  
				 <div class="form-group">
		       <label for="productName"><b>상품명</b></label>
		       <input type="text" name="productName" class="form-control" id="productName" value="${productVo.productName}" required >
		     </div>
			   <div class="form-group">
		       <label for="realPrice"><b>상품원가</b></label>
		       <input type="text" class="form-control" id="realPrice" value="${productVo.realPrice}" name="realPrice" maxlength="10" required >
			   </div> 
		     <div class="form-group">
		       <label for="mainPrice"><b>판매가격</b></label>
		       <input type="text" class="form-control" id="mainPrice" value="${productVo.mainPrice}" name="mainPrice" maxlength="10" onkeyup="pointCalc(this.value)" required >
			   </div>
			   <div class="form-group">
		       <label for="point">
		       	<b>적립금</b>&nbsp;&nbsp;&nbsp;&nbsp;
		       ...
		       <input type="text" class="form-control" id="point" value="${productVo.point}" name="point" maxlength="10" readonly >
			   </div>
			   <div class="form-group">
		      ...
		      <input type="text" class="form-control" id="disPrice" value="${productVo.disPrice}" name="disPrice" maxlength="10" style="width: 46%;" onkeyup="pointCalc(this.value)" readonly >
		 	  </div>
				<div class="form-group">
			      <label for="file"><b>메인이미지</b>(${productVo.productMainImg})<img src="${ctp}/data/shop/product/${productVo.productMainImg}" width="150px"/></label>
			      <br/>-- 수정시 메인이미지를 다시 올려주세요 -- 
		      ...
		      <textarea rows="6" cols="120" class="form-control" id="productContent" placeholder="상품 상세 설명" name="productContent" required >${productVo.productContent}</textarea>
		    </div>  
				<div class="form-group">
		      <label for="productContent2"><b>상품 세부정보</b></label>
		      <textarea rows="6" cols="120" class="form-control" id="productContent2" placeholder="상품 세부 정보" name="productContent2" required >${productVo.productContent2}</textarea>
		    </div>  
		    <div class="form-group">
		       <label for="productSubImg"><b>상품상세사진</b></label>
		       <textarea class="form-control" rows="5" id="CKEDITOR" name="productSubImg" >${productVo.productSubImg}</textarea>
		    </div>
		    <div style="text-align: center;"><br/>
		   		<button type="button" class="btn btn-warning" onclick="fCheck()" style="width:120px;">상품수정</button> &nbsp;
		 	  </div>
     	...
     	<form name="optionForm${vo.idx}" method="post">
				<div class="form-group"><br/>
					<font size="4"><b>상품옵션수정</b></font>&nbsp;&nbsp;
	     		<br/><br/>
   				<c:forEach var="vo" items="${optionVos}">
	   				<div class="form-group">
		          <label for="optionName"><b>상품사이즈${vo.idx}</b></label>
		          <br/><br/>
		       	  <select name="optionName" id="optionName${vo.idx}" class="form-control">
	     	  	  	 <option value="Free"${vo.optionName == 'Free' ? 'selected' : '' }>Free</option>
	     	  	  	 <option value="S"${vo.optionName == 'S' ? 'selected' : '' }>S</option>
	     	  	  	 <option value="M"${vo.optionName == 'M' ? 'selected' : '' }>M</option>
	     	  	  	 <option value="L"${vo.optionName == 'L' ? 'selected' : '' }>L</option>
	     	  	  	 <option value="XL"${vo.optionName == 'XL' ? 'selected' : '' }>XL</option>
	     	  	  	 <option value="XXL"${vo.optionName == 'XXL' ? 'selected' : '' }>XXL</option>
		          </select>
	   				</div>  
	   				<div class="form-group">
			        <label for="optionColor"><b>색상</b></label>
			        <input type="text" class="form-control" id="optionColor${vo.idx}" value="${vo.optionColor}" name="optionColor" required >
			  		</div> 
	       		<div class="form-group">
			        <label for="optionNum"><b>재고</b></label>&nbsp;&nbsp;
			        <input type="number" name="optionNum" id="optionNum${vo.idx}" value="${vo.optionNum}" class="form-control" required >
						</div>
						<input type="hidden" name="idx" value="${vo.idx}"/>
						<div style="text-align: center;"><br/>
			   			<button type="button" class="btn btn-warning" onclick="OCheck(${vo.idx})" style="width:120px;">옵션수정</button> &nbsp;
			   			<button type="button" class="btn btn-warning" onclick="ODel(${vo.idx})" style="width:120px;">옵션삭제</button> &nbsp;
				 	 	</div>
					</c:forEach>
				 	</div> 	
					</form>
